...
# Parse optional --paths flag; defaults to /api when not supplied
API_PATHS=("/api")
...
if [[ -n "$ORIGIN_SECRET" && -n "$API_GATEWAY_URL" ]]; then
  ...
  ENDPOINTS_TFVARS="$FOUNDATION_TF/service-endpoints.auto.tfvars"

  echo "🔗 Wiring API Gateway to CloudFront..."
  echo "   Writing ${ENDPOINTS_TFVARS}"

  cat > "$ENDPOINTS_TFVARS" <<EOF
# Auto-generated by deploy-service.sh — do not edit manually.
# Gitignore this file if the origin_verify_secret is security-sensitive.
api_gateway_url      = "${API_GATEWAY_URL}"
origin_verify_secret = "${ORIGIN_SECRET}"
api_path_prefixes    = ${API_PATHS_JSON}
EOF
